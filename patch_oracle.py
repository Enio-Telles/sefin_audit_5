...
# 1. Add imports
import_replacement = """from core.utils import validar_cnpj, ler_sql, extrair_parametros_sql, normalizar_colunas
...
db_conn_replacement = """        dsn = oracledb.makedsn(request.connection.host, request.connection.port, service_name=request.connection.service)
        ...
        with ExecutionTimer(trace, stage="db_connection"):
            ...
            # Explicitly enter and exit to manage connection in context properly since we replaced the with block
            conexao.__enter__()

        try:"""
content = re.sub(db_conn_pattern, db_conn_replacement, content)

# We need to make sure conexao.__exit__ is called at the end of the extraction. We'll find the return and append the exit, along with saving the trace.
...
# Replacing the inner loop execution
