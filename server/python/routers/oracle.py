...
from core.utils import validar_cnpj, ler_sql, extrair_parametros_sql
...
async def extract_oracle_data(request: ExtractionRequest):
    ...
    output_path = (
        Path(request.output_dir) / cnpj_limpo
        if cnpj_limpo
        else Path(request.output_dir)
    )
    ...
    try:
        ...
        try:
                ...
                with ExecutionTimer(trace, stage="execute_query", query=query_name):
                    try:
                        ...
                        for p in list(bind_vars.keys()):
                            if bind_vars[p] is None and p.lower() not in (
                                "cnpj",
                                "cnpj_raiz",
                            ):
                                bind_vars[p] = ""
                        ...
                        with conexao.cursor() as cursor:
                            ...
                            parquet_name = (
                                f"{query_name}_{cnpj_limpo}.parquet"
                                if cnpj_limpo
                                else f"{query_name}.parquet"
                            )
            ...
            if request.include_auxiliary and request.auxiliary_queries_dir:
                ...
                if aux_sql_dir.exists() and aux_sql_dir.is_dir():
                    ...
                    logger.info(
                        "[extract] Executando %d consultas auxiliares de '%s'",
                        len(aux_sql_files),
                        aux_sql_dir,
                    )
                    for aux_file in aux_sql_files:
                        ...
                        with ExecutionTimer(
                            trace, stage="execute_aux_query", query=aux_name
                        ):
                            try:
                                ...
                                for p in list(aux_bind.keys()):
                                    if aux_bind[p] is None and p.lower() not in (
                                        "cnpj",
                                        "cnpj_raiz",
                                    ):
                                        aux_bind[p] = ""
                                ...
                                with conexao.cursor() as cursor:
                                    ...
                                    aux_parquet_path = (
                                        aux_output_path / aux_parquet_name
                                    )

                                    tmp_parquet_path = (
                                        f"{aux_parquet_path}.tmp_{uuid.uuid4()}"
                                    )
                                    writer = None
                                    total_rows = 0
                                    try:
                                        ...
                                        os.replace(
                                            tmp_parquet_path, str(aux_parquet_path)
                                        )
                                    except Exception:
                                        if writer:
                                            writer.close()
        ...
        return {
            ...
            "trace_dir": str(output_path / "_audit" / trace.execution_id),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Driver Oracle (oracledb) não instalado"
        )
    except Exception as e:
        trace.add_event("error", "error", str(e))
        try:
