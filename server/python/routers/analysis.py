...
async def run_audit_pipeline_bg(
    ...
    try:
        ...
        if not saved_password:
            raise HTTPException(
                status_code=400, detail="Senha Oracle ausente no Cofre."
            )

        dsn = oracledb.makedsn(
            "exa01-scan.sefin.ro.gov.br", 1521, service_name="sefindw"
        )
        ...
        with conexao.cursor() as cursor:
            ...
            if not sql_files:
                raise Exception(
                    f"Nenhum arquivo .sql encontrado no diretório: {dir_sql}"
                )
            arquivos_extraidos = []
            erros = []
                ...
                try:
                    ...
                    for p in params:
                        if p.lower() == "cnpj":
                            ...
                            and req.data_limite_processamento
                        ):
        ...
        try:
            ...
            for fp in fontes_produto:
                if fp.exists():
                    try:
                        n_rows = (
                            pl.scan_parquet(str(fp)).select(pl.len()).collect().item()
                        )
                        if n_rows > 0:
            ...
            base_detalhes_path = (
                dir_analises / f"base_detalhes_produtos_{cnpj_limpo}.parquet"
            )
            if fontes_com_dados:
                if (
                ...
                base_rows = (
                    pl.scan_parquet(str(base_detalhes_path))
                    .select(pl.len())
                    .collect()
                    .item()
                ...
                if (
                    produto_pipeline_em_modo_compatibilidade()
                    and not base_detalhes_path.exists()
                ):
                    logger.warning(
            ...
            targets = [
                ...
                (
                    f"mapa_auditoria_desagregados_{cnpj_limpo}.parquet",
...
async def importar_fatores_excel(
    ...
    try:
        ...
        if not required_cols.issubset(set(df_excel.columns)):
            raise HTTPException(
                status_code=400, detail="Colunas obrigatórias ausentes no Excel."
            )
