...
async def export_to_excel(request: ExcelExportRequest):
    ...
    try:
        ...
        return {"success": False, "results": [{"status": "error", "message": "Caminho de saída inválido"}]}
    if not _is_path_allowed(output_path):
        return {"success": False, "results": [{"status": "error", "message": "Acesso ao diretório de saída não permitido"}]}
