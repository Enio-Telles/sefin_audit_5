...
def _aplicar_desagregacao_codigos(df_base: pl.DataFrame) -> pl.DataFrame:
    ...
    # ⚡ Bolt Optimization: map_elements boundary crossing over unique items only
...
def _build_codigos_multidescricao(df_indexados: pl.DataFrame) -> pl.DataFrame:
    ...
    if "descricao_normalizada" not in df_indexados.columns:
        df_indexados = df_indexados.join(unique_desc, on="descricao", how="left").rename({"__descricao_normalizada_new": "descricao_normalizada"})
    else:
