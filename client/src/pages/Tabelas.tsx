...
export default function Tabelas() {
  ...
  const [selectedFile, setSelectedFile] = useState<ParquetFileInfo | null>(
    null
  );
  ...
  >({});
  const [isLoadingUniqueValues, setIsLoadingUniqueValues] = useState<
  ...
  const [isLoadingSuggestionSummary, setIsLoadingSuggestionSummary] =
    useState(false);
  ...
  const getPersistenceKey = (filePath: string) =>
    `sefin-table-prefs-${filePath.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  const inferSuggestionContext = useCallback((filePath: string) => {
    ...
    let match = normalized.match(
      /pares_descricoes_similares_light_(\d{14})\.parquet$/i
    );
    if (match) return { metodo: "light" as const, cnpj: match[1] };
    match = normalized.match(
      /pares_descricoes_similares_faiss_(\d{14})\.parquet$/i
    );
  ...
  const handleLoadReferences = useCallback(async () => {
    try {
      ...
      toast.error("Erro ao buscar referências", { description: err.message });
    }
  }, [handleLoadFiles]);

  const handleOpenFile = useCallback(
    async (file: ParquetFileInfo, resetPagination = true) => {
      ...
      if (resetPagination) {
        ...
        if (loadedPrefs?.columnStyles)
          setColumnStyles(loadedPrefs.columnStyles);
      }
    ...
    if (rawFilePathParam) {
      ...
      if (filePathParam.includes("undefined") || filePathParam === "null") {
        ...
        toast.error("Erro no caminho do arquivo", {
          description:
            "O caminho fornecido é inválido (backend indisponível ou erro de processamento).",
        });
        return;
      }
    ...
    const loadSummary = async () => {
      try {
        ...
        if (suggestionContext.metodo !== "lexical") {
          ...
          const cache =
            suggestionContext.metodo === "light"
              ? status.caches?.light
          ...
          if (
            ...
            minSemanticScore = Number(
              cache?.min_semantic_score ?? minSemanticScore
            );
          }
        }

        const response = await getParesGruposSimilares(
          suggestionContext.cnpj,
          suggestionContext.metodo,
          false,
          {
            topK: Number.isFinite(topK) ? topK : 8,
            minScore: Number.isFinite(minScore) ? minScore : 0.72,
            minSemanticScore: Number.isFinite(minSemanticScore)
              ? minSemanticScore
              : 0.32,
            page: 1,
            pageSize: 1,
  ...
  const handleFetchUniqueValues = async (col: string) => {
    ...
    try {
      ...
      toast.error(`Erro ao buscar valores únicos`, {
  ...
  const handleExportCurrent = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/python/export/excel-download?file_path=${encodeURIComponent(selectedFile.path)}`
      );
              ...
              <div className="pt-2 flex flex-wrap gap-2 items-center">
                ...
                {recentFolders.map((folder, i) => (
                  ...
                  >
                    ...
                    <button
                      className="ml-1.5 opacity-0 group-hover:opacity-100 p-0.5 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-sm"
                  ...
                  {files.map(file => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 shrink-0 text-xs text-muted-foreground">
                        ...
                        <button
                          className="opacity-0 group-hover:opacity-100 p-1 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-sm"
                          onClick={e => {
                            e.stopPropagation();
                            openParquetInNewTab(file.path);
                          }}
                          aria-label="Abrir tabela em nova guia"
                        >
        ...
        <>
          <Card className="border shadow-sm flex flex-col flex-1 min-h-0 overflow-hidden">
            <CardContent className="p-3 border-b bg-muted/30 shrink-0">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant="secondary" className="text-xs">
                    Visualizacao atual: {tableData?.filtered_rows ?? 0} de{" "}
                    {tableData?.total_rows ?? 0} no arquivo
                  </Badge>
                  {suggestionSummary ? (
                    <>
                      <Badge
                      ...
                      >
                        Fila operacional: {suggestionSummary.totalVisible}{" "}
                        visiveis
                      </Badge>
                      <Badge
                        variant="outline"
                        className="border-border/70 bg-background/70 text-xs text-muted-foreground"
                      >
              ...
              {suggestionSummary ? (
                <div className="mt-3 rounded-lg border border-border/70 bg-background/60 px-3 py-2 text-xs text-muted-foreground">
                  <div>
                    Este arquivo pertence a sugestoes de similaridade no modo{" "}
                    <strong className="text-foreground">
                      {suggestionSummary.metodo}
                    </strong>
                    .
                  </div>
                  <div className="mt-1">
                    A fila operacional considera{" "}
                    <strong className="text-foreground">
                      {suggestionSummary.totalVisible}
                    </strong>{" "}
                    pares visiveis agora, enquanto o parquet contem{" "}
                    <strong className="text-foreground">
                      {suggestionSummary.totalFile}
                    </strong>{" "}
                    pares brutos.
                  </div>
                  <div className="mt-1">
                    A visualizacao atual desta tela usa os filtros e a paginacao
                    locais sobre o arquivo bruto.
                    {isLoadingSuggestionSummary
            ...
            >
              ...
              >
                <table
                  className="w-full text-xs border-collapse absolute top-0 left-0"
                  style={{ tableLayout: "fixed" }}
                >
                  <thead className="sticky top-0 z-30 shadow-sm bg-muted">
                    <tr className="bg-muted">
                      <th className="px-3 py-2 text-left font-semibold text-muted-foreground w-12 border-b border-r bg-muted sticky left-0 z-40">
                        #
                      </th>
                      {localColumns
                        ...
                        .map(col => (
                          ...
                          >
                            <div className="flex items-center gap-1.5 justify-between">
                              ...
                              <DropdownMenu
                                onOpenChange={open => {
                                  if (open) handleFetchUniqueValues(col);
                                }}
                              >
                                <DropdownMenuTrigger asChild>
                                  <button
                                    className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-background/50 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      ...
                      {showReviewActions && (
                        <th className="px-3 py-2 text-center font-semibold text-muted-foreground border-b border-r bg-muted sticky right-10 z-40 w-[260px]">
                          Ações de Revisão
                        </th>
                      )}
                    ...
                    {showFilterRow && (
                      <tr className="bg-muted/80 sticky top-[33px] z-30">
                        <th className="px-3 py-1.5 border-b border-r bg-muted sticky left-0 z-40">
                        ...
                        {localColumns
                          ...
                          .map(col => (
                            <th
                              key={col}
                              className="px-2 py-1.5 border-b border-r bg-muted/80 font-normal"
                            >
                              <Input
                                placeholder="Filtrar..."
                                value={filters[col] || ""}
                        ...
                        >
                              ...
                              return (
                                <td
                                  ...
                                  style={{
                                    ...
                                    backgroundColor:
                                      rStyle.backgroundColor || cStyle.cellBg,
                                  }}
                                  onDoubleClick={() =>
                                    handleCellDoubleClick(rowIdx, col)
                                  }
                                >
                                  {isEditing ? (
                                    <Input
                              ...
                              <div className="flex items-center gap-1.5 justify-center h-full">
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    if (!currentCnpj) {
                                      toast.error("CNPJ não detectado.");
                                      return;
                                    }
                                    window.open(
                                      `/unificar/${currentCnpj}/${codigo}`,
                                      "_blank"
                                    );
                                  }}
                                >
                          ...
                          <td className="px-1 py-1.5 border-b border-r shrink-0 w-10 sticky right-0 bg-background/95 z-10">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <button
                                  className="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-muted transition-opacity focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                              ...
                              <DropdownMenuContent align="end" className="w-56">
                                <DropdownMenuItem
                                  onClick={() => openColorDialog("row", rowIdx)}
                                >
                                  <Paintbrush className="h-3 w-3 mr-2" /> Cores
                                  da Linha
                                </DropdownMenuItem>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t bg-muted/30 shrink-0">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  className="h-7 w-7"
                  disabled={
                    page >= (tableData?.total_pages || 1) || isLoadingTable
                  }
      ...
      <Dialog open={showColorDialog} onOpenChange={setShowColorDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              Personalizar Cores —{" "}
              {colorTarget?.type === "column"
                ? `Coluna: ${colorTarget.id}`
                : `Linha: ${(colorTarget?.id as number) + 1}`}
            </DialogTitle>
          </DialogHeader>
