...
interface AuditResultViewProps {
  result: AuditPipelineFinalResult;
  elapsed?: string;
}
...
export function AuditResultView({ result, elapsed }: AuditResultViewProps) {
  ...
  const statusResumo = (statusQuery.data?.resumo ||
    null) as ProdutoAnaliseStatusResumo | null;
  ...
  const faissSummaryQuery = useQuery({
    ...
    queryFn: () =>
      getParesGruposSimilares(cleanCnpj, "faiss", false, {
        ...
        minScore: Number.isFinite(faissCacheMinScore)
          ? faissCacheMinScore
          : 0.62,
        page: 1,
        pageSize: 1,
  ...
  const lightSummaryQuery = useQuery({
    queryKey: [
      "audit-produto-light-summary",
      cleanCnpj,
      lightCache?.generated_at_utc ?? "",
    ],
    queryFn: () =>
      getParesGruposSimilares(cleanCnpj, "light", false, {
        topK: Number.isFinite(lightCacheTopK) ? lightCacheTopK : 8,
        minScore: Number.isFinite(lightCacheMinScore)
          ? lightCacheMinScore
          : 0.72,
        page: 1,
        pageSize: 1,
  ...
  const handleGenerateSuggestions = async () => {
    ...
    try {
      const response = await getParesGruposSimilares(
        cleanCnpj,
        suggestionMode === "faiss" ? "faiss" : "light",
        true,
        {
          ...
          minScore: Math.max(
            ...
            Math.min(
              0.98,
              Number(suggestionMinScore) ||
                (suggestionMode === "faiss" ? 0.62 : 0.72)
            )
          ),
          page: 1,
          pageSize: 50,
      ...
      const totalVisible = Number(
        response.total_filtered ?? response.total ?? 0
      );
      const totalFile = Number(response.total_file ?? totalVisible);
      setDownloadMsg(
        `Sugestoes ${suggestionMode === "faiss" ? "FAISS" : "leves"} geradas (${totalVisible} visiveis de ${totalFile} no arquivo).`
      );
      await Promise.all([
        vectorStatusQuery.refetch(),
        lightSummaryQuery.refetch(),
        faissSummaryQuery.refetch(),
      ]);
    } catch (err: any) {
      setDownloadMsg(
        err?.message ||
          `Erro ao gerar sugestoes ${suggestionMode === "faiss" ? "FAISS" : "leves"}`
      );
    } finally {
  ...
  const handleClearSuggestions = async () => {
    ...
    setSuggestionLoading(true);
    try {
      await clearVectorizacaoCache(
        cleanCnpj,
        suggestionMode === "faiss" ? "faiss" : "light"
      );
      setDownloadMsg(
        `Cache de sugestoes ${suggestionMode === "faiss" ? "FAISS" : "leves"} removido.`
      );
      await Promise.all([
        vectorStatusQuery.refetch(),
        lightSummaryQuery.refetch(),
      ...
      <div className="min-w-0 flex-1 space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {file.analise && (
            <Badge
              variant="outline"
              className="ml-2 border-border/70 bg-background/70 py-0 text-[9px] text-muted-foreground"
            >
              {file.analise}
            </Badge>
          )}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 lg:grid-cols-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {result.arquivos_extraidos?.length || 0}
                </p>
        ...
        <Card className="border-border/70 bg-card/95 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {result.arquivos_analises?.length || 0}
                </p>
        ...
        <Card className="border-border/70 bg-card/95 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {result.arquivos_relatorios?.length || 0}
                </p>
        ...
        <Card className="border-border/70 bg-card/95 shadow-sm">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              <div
                className={`rounded-lg p-2 ${(result.erros?.length || 0) > 0 ? "bg-orange-500/10" : "bg-emerald-500/10"}`}
              >
          ...
          <CardContent>
            <div className="space-y-3">
              <p className="text-sm leading-6 text-muted-foreground">
                A entrada principal da analise de produtos agora fica
                concentrada em um workspace unico. A revisao final continua
                disponivel, mas os controles tecnicos ficaram recolhidos para
                reduzir a carga visual.
              </p>
              ...
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                {[
                  ...
                  {
                    label: "Verificados",
                    value: statusResumo?.verificados ?? 0,
                  },
                  {
                    label: "Consolidados",
                    value: statusResumo?.consolidados ?? 0,
                  },
                  {
                    label: "Decididos",
                    value:
                      (statusResumo?.separados ?? 0) +
                      (statusResumo?.decididos_entre_grupos ?? 0),
                  },
                ].map(item => (
                  <div
                    key={item.label}
                    className="rounded-xl border border-border/70 bg-accent/35 px-3 py-2"
                  >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  variant="outline"
                  className="gap-2"
                  onClick={() => openAnaliseProdutos("fatores")}
                >
              ...
              <Collapsible
                open={advancedProductsOpen}
                onOpenChange={setAdvancedProductsOpen}
              >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-muted-foreground">
                        Runtime, sugestoes opcionais, artefatos tecnicos e
                        arquivos brutos continuam disponiveis aqui.
                      </div>
                    </div>
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" size="sm" className="gap-2">
                        {advancedProductsOpen
                          ? "Ocultar detalhes"
                          : "Mostrar detalhes"}
                        <ChevronDown
                          className={`h-4 w-4 transition-transform ${advancedProductsOpen ? "rotate-180" : ""}`}
                        />
                      </Button>
                    </CollapsibleTrigger>
                  </div>
                    ...
                    <div className="rounded-xl border border-border/70 bg-accent/30 px-3 py-3">
                      ...
                      <div className="text-sm text-foreground">
                        {runtimeStatus ? (
                          <>
                            <span>
                              Modo: <strong>fluxo atual</strong>.
                            </span>
                            <span className="ml-2">
                              Somente artefatos do sistema novo sao exibidos.
                            </span>
                          </>
                        ) : (
                      ...
                      {runtimeStatus ? (
                        <div className="mt-2 text-xs text-muted-foreground">
                          ...
                          {
                            Object.values(runtimeStatus.files || {}).filter(
                              item => item.exists
                            ).length
                          }
                          /{Object.keys(runtimeStatus.files || {}).length}
                        </div>
                      ) : null}
                      <div className="mt-2 flex flex-wrap gap-2">
                        {runtimeStatus
                          ? Object.entries(runtimeStatus.files || {}).map(
                              ([key, item]) => (
                                <span
                                  key={key}
                      ...
                      <div className="mt-3 rounded-lg border border-border/70 bg-background/40 px-3 py-2">
                        ...
                        <div className="mt-1 text-xs text-muted-foreground">
                          ...
                          .{" "}
                          {suggestionMode === "faiss"
                            ? faissModeStatus?.message ||
                              "FAISS desativado ate ser solicitado manualmente."
                            : lightModeStatus?.message ||
                              "Modo leve desativado ate ser solicitado manualmente."}
                        </div>
                        {activeCache?.generated_at_utc ? (
                          <div className="mt-2 text-xs text-muted-foreground">
                            {activeSummaryQuery?.isLoading ? (
                              ...
                              <span>
                                ...
                                de{" "}
                                <strong className="text-foreground">
                                  {activeFileCount || activeVisibleCount}
                                </strong>{" "}
                                no arquivo.
                              </span>
                            )}
                        ...
                        <div className="mt-1 text-[11px] text-muted-foreground">
                          O total visivel exclui pares ocultados por status de
                          analise. O total do arquivo representa o bruto gravado
                          no parquet.
                        </div>
                      </div>
                    </div>
                      ...
                      <ActionGroup title="Revisao">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            window.open(
                              `/analise-produtos?${params.toString()}`,
                              "_blank"
                            );
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            window.open(
                              `/revisao-fatores?cnpj=${cleanCnpj}`,
                              "_blank"
                            );
                          }}
                        >
                      ...
                      <div className="w-full rounded-lg border border-border/70 bg-background/40 p-3">
                        <div className="grid gap-3 md:grid-cols-[minmax(0,1.4fr)_96px_112px]">
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              onChange={event =>
                                setSuggestionTopK(event.target.value)
                              }
                            />
                          </div>
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              onChange={event =>
                                setSuggestionMinScore(event.target.value)
                              }
                            />
                          </div>
                        </div>
                        <div className="mt-3 grid gap-3 md:grid-cols-3">
                          ...
                          <Button
                            ...
                            className="h-8 w-full gap-1.5"
                            onClick={() =>
                              openAnaliseFileByName(selectedSuggestionFile)
                            }
                          ...
                          <Button
                            ...
                            disabled={
                              suggestionLoading || suggestionMode === "off"
                            }
                          >
                      ...
                      <ScrollArea className="h-[220px]">
                        <div className="space-y-1.5">
                          {visibleProductFiles.length > 0 ? (
                            visibleProductFiles.map((file, i) => (
        ...
        <Card className="border-border/70 bg-card/95 shadow-sm">
          ...
          <CardContent>
            ...
            <div className="flex items-center justify-between">
              <code className="max-w-[250px] truncate text-[10px] text-muted-foreground">
                {result.dir_parquet}
              </code>
              <Button
                variant="outline"
          ...
          <CardContent>
            <div className="space-y-2">
              {result.arquivos_relatorios?.map((doc, i) => (
                ...
                >
                  <div
                    className={`rounded-md p-1.5 ${doc.tipo?.includes("Word") || false ? "bg-blue-500/10" : "bg-gray-500/10"}`}
                  >
                    <FileText
                      className={`h-4 w-4 ${doc.tipo?.includes("Word") || false ? "text-blue-500" : "text-gray-500"}`}
                    />
                  </div>
            ...
            <Separator className="my-3" />
            <div className="flex items-center justify-between">
              <code className="max-w-[350px] truncate text-[10px] text-muted-foreground">
            ...
            <ScrollArea className="h-[120px]">
              <div className="space-y-1 font-mono text-xs">
                {result.erros?.map((erro, i) => (
                  <p key={i} className="text-orange-600 dark:text-orange-400">
                    {erro}
                  </p>
                ))}
              </div>
